...
const CreateEventForm = ({ onSuccess, onCancel }) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // Validate that registration deadline is in the future
      if (registrationDeadlineDateTime <= new Date()) {
      ...
      const message = error.response?.data?.msg || 'Failed to create event';
      toast.error(message);
    } finally {
      setLoading(false);
    }
  ...
  const categories = [
    'academic', 'cultural', 'sports', 'workshop', 'seminar', 'social', 'other'
  ];
